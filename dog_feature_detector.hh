...
public:
	...
	*/
	void detect(const cv::Image<float> &im, int level=-1){
		...
		if(level==-1){
			...
			//std::cout<<"calculating scale spaces"<<std::endl;
			...
			for(int i=_spaces.getNumScales()-1; i>=0; i--){
				//std::cout<<"Looking for features at scale "<<i<<std::endl;
				gt.realloc(_spaces.getImage(i,0)); //.width-2, _spaces.getImage(i,0).height-2);
				lt.realloc(gt); //gt.width, gt.height);
				tmp.realloc(gt); //gt.width, gt.height);
				for(int j=1; j<_spaces.getNumSubScales()-1; j++){
					findPeaks3Level(_spaces.getImage(i,j-1),
							        _spaces.getImage(i,j),
									_spaces.getImage(i,j+1),
									gt, lt, tmp,
									_dogThresh, i, j);
				}
			}
			...
			gt.realloc(_spaces.getImage(level,0));
			lt.realloc(gt); //gt.width, gt.height);
			tmp.realloc(gt); //gt.width, gt.height);
			for(int j=1; j<_spaces.getNumSubScales()-1; j++){
	...
	void filter() {
		...
		for(iterator it = _features.begin(); 
			...
			if(filterEdge(_spaces.getImage(it->level, it->subLevel), cvRound(it->x), cvRound(it->y), _edgeRatio)){
				iterator it2 = it;
				it++;
				_features.erase(it2);
			...
			// this step ought to spawn other features if the orientation is ambiguous
				...
				iterator it2 = it;
				++it;
				//std::cout<<"Feature "<<i<<" - "<<*it2<<" - "<<std::endl;
				cvGetQuadrangleSubPix(&_spaces.getIx(it2->level,it2->subLevel), &Ix, &A);
				cvGetQuadrangleSubPix(&_spaces.getIy(it2->level,it2->subLevel), &Iy, &A);
	...
	*/
	...
	void findPeaks3Level(const cv::Image<float> & below,
		...
		// threshold first 
		cvInRangeS(&current, cvScalar(-dogThresh), cvScalar(dogThresh), &gt);
		cvNot(&gt, &gt);

		lt = gt;

		// current level
		cvIsLocalMax2(current, current, gt, false, 3, &gt);
		cvIsLocalMax2(current, below, gt, true, 3, &gt);
		cvIsLocalMax2(current, above, gt, true, 3, &gt);

		cvIsLocalMin2(current, current, lt, false, 3, &lt);
		cvIsLocalMin2(current, below, lt, true, 3, &lt);
		cvIsLocalMin2(current, above, lt, true, 3, &lt);
	...
	/*void findPeaks(const cv::DoGScaleSpace<float> &dog,  float dogThresh){
		...
		for(int i=0; i<dog.getNumScales(); i++){
			...
			gt.realloc(dog.getImage(i,0).width-2, dog.getImage(i,0).height-2);
			lt.realloc(gt.width, gt.height);
			tmp.realloc(gt.width, gt.height);
	...
	bool subPixelLocalize(feature_t & p, int nadjustments=2){
		...
		while (needToAdjust) {
			...
			p.x = x + adjX;
			p.y = y + adjY;
			p.subLevel = cvRound(sublevel + adjS + 0.5);
	...
	void orientFeature(feature_t &p){
		...
		int h = _spaces.getGaussian(p.level,p.subLevel).getHeight();
		int xmin = cvRound(MAX(1, p.x-radius));
		int ymin = cvRound(MAX(1, p.y-radius));
		int xmax = cvRound(MIN(w-1, p.x+radius));
		int ymax = cvRound(MIN(h-1, p.y+radius));

		for(int i=xmin; i<xmax; i++){
			for(int j=ymin; j<ymax; j++){
				int relX = cvRound(i - p.x);
				int relY = cvRound(j - p.y);
				...
				hist[bin] += cvRound(weight*gWeight);
			}
		}
	...
	void binFeature(const int &x, const int &y, const cv::Image<float> & space, const int &nbins, 
		...
		bin = cvRound(nbins*angle*.5/M_PI);
	}
