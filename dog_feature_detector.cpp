...
void cvDetectFeaturesDoG( CvArr * src, CvMemStorage * storage, CvSeq * features, float dogThresh=3.0, float edgeRatio=5.5, float contrastThresh=0.03, float sigma=1.6,    int minImageSize=16, int nsub=1){
	cv::DoGPyramid<float> _spaces( minImageSize, nsub );
	...
	for(int i=_spaces.getNumScales()-1; i>=0; i--){
		...
		for(int j=1; j<_spaces.getNumSubScales()-1; j++){
			icvFindPeaks3Level(_spaces.getImage(i,j-1),
	...
	void icvFindPeaks3Level(const cv::Image<float> & below,
